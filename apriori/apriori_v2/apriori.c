...
int get_transcation(int counter1, int counter2, struct linkPointer *transcation1, struct linkPointer *transcation2, struct linkPointer **transcation_head) {
	struct linkPointer *transcation = malloc(sizeof(struct linkPointer));
	transcation->value = -1;

	int counter = 0;
	while(transcation1 != NULL && transcation2 != NULL) {
		if(counter1+counter < MIN_SUPPORT || counter2+counter < MIN_SUPPORT) {
			break;
		}
